...
| Profile-page editor coverage | UI gap, not schema gap | After retiring the Directory inline-expand form on 2026-05-17 (commit `95304a3`), `profile.js` covers Role / Phone / Address / Line manager / Access level / Auth0 ID / Google accounts / Payroll editors. Display name, Aliases, Start date, Notes, Line manager (editor), Access level (editor), Suspend toggle, and Delete Person were wired later the same day. **Still missing** (Google-account lifecycle UI): per-account alias management (Add alias / Remove alias / Alias → group), Convert primary account to group, Promote alt to primary (rename). Worker actions exist (`user-alias-add` added today; `user-alias-remove`, `alias-to-group`, `convert-to-group`, `rename-user` all existed before). |
| Delete + 21-day delayed group conversion | Not implemented | Workflow the user wants: delete a primary Google account, then convert the freed email into a forwarding group 21 days later (matches Google's own grace window on the renamed primary). Needs a new pending-operations record + a scheduled run that fires the existing `convert-to-group` action against the target email + member list when the timer expires. Existing `pending-transfers.json` + `scripts/process_pending_transfers.py` are the prior art to extend. |
