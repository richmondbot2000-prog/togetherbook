...
**Mechanical inventory (auto-generated, drift sentry).** The block below is rewritten by `python3 scripts/generate_canonical_tables.py` from `.github/workflows/*.yml` directly. If it disagrees with the manual table above, the manual table is stale — fix it.

<!-- AUTO:workflows BEGIN -->

| Workflow | Cadence (UTC) | Secrets it uses |
|---|---|---|
| `birthday-posts.yml` | daily 05:00 UTC (+manual) | — |
| `email-payroll-monthly.yml` | monthly 09:00 UTC on day 1 (+manual) | `SMTP_PASSWORD`, `SMTP_USERNAME` |
| `probe-activity-columns.yml` | manual only | `FABRIC_CLIENT_SECRET` |
| `probe-lead-attribution.yml` | manual only | `FABRIC_CLIENT_SECRET` |
| `probe-lead-outcomes.yml` | manual only | `FABRIC_CLIENT_SECRET` |
| `process-pending-transfers.yml` | `40 * * * *` (+manual) | `WORKSPACE_DELEGATE_USER`, `WORKSPACE_DELEGATE_USER_TOGETHERLOANS`, `WORKSPACE_SERVICE_ACCOUNT_JSON`, `WORKSPACE_SERVICE_ACCOUNT_JSON_TOGETHERLOANS` |
| `reconcile-people.yml` | daily 06:30 UTC (+manual) | — |
| `refresh-1st-contact.yml` | hourly 6-23 UTC :00 (+manual) | `FABRIC_CLIENT_SECRET` |
| `refresh-auth0.yml` | hourly 6-23 UTC :00 (+manual) | `AUTH0_CLIENT_ID`, `AUTH0_CLIENT_SECRET` |
| `refresh-brandwatch.yml` | hourly 6-23 UTC :00 (+manual) | `REDDIT_CLIENT_ID`, `REDDIT_CLIENT_SECRET`, `SCRAPERAPI_KEY`, `SMTP_PASSWORD`, `SMTP_USERNAME`, `YOUTUBE_API_KEY` |
| `refresh-brokers.yml` | hourly 6-23 UTC :35 (+manual) | `FABRIC_CLIENT_SECRET` |
| `refresh-comms.yml` | hourly 6-23 UTC :05 (+manual) | `FABRIC_CLIENT_SECRET` |
| `refresh-directory.yml` | hourly 6-23 UTC :00 (+manual) | `WORKSPACE_DELEGATE_USER`, `WORKSPACE_SERVICE_ACCOUNT_JSON`, `WORKSPACE_TENANTS` |
| `refresh-discord.yml` | hourly 6-23 UTC :45 (+manual) | `DISCORD_TOKEN`, `SLACK_WEBHOOK_URL` |
| `refresh-groups.yml` | hourly 6-23 UTC :20 (+manual) | `WORKSPACE_DELEGATE_USER`, `WORKSPACE_SERVICE_ACCOUNT_JSON`, `WORKSPACE_TENANTS` |
| `refresh-hibp.yml` | every 6h, :20 UTC (+manual) | `HIBP_API_KEY`, `SLACK_WEBHOOK_URL` |
| `refresh-lookalike.yml` | daily 05:00 UTC (+manual) | `SLACK_WEBHOOK_URL` |
| `refresh-pending-conversions.yml` | daily 06:30 UTC (+manual) | `WORKSPACE_DELEGATE_USER`, `WORKSPACE_SERVICE_ACCOUNT_JSON` |
| `refresh-pipeline-samples.yml` | hourly 6-23 UTC :50 (+manual) | `FABRIC_CLIENT_SECRET` |
| `refresh-pipeline.yml` | hourly 6-23 UTC :45 (+manual) | `FABRIC_CLIENT_SECRET` |
| `refresh-row-counts.yml` | hourly 6-23 UTC :00 (+manual) | `FABRIC_CLIENT_SECRET` |
| `refresh-source-quality.yml` | daily 07:05 UTC (+manual) | `FABRIC_CLIENT_SECRET` |
| `refresh-staff-activity.yml` | daily 12:17 UTC (+manual) | `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_API_TOKEN`, `D1_ACTIVITY_DB_ID`, `FABRIC_CLIENT_SECRET`, `WORKSPACE_DELEGATE_USER`, `WORKSPACE_SERVICE_ACCOUNT_JSON` |
| `refresh-telegram.yml` | hourly 6-23 UTC :40 (+manual) | `SLACK_WEBHOOK_URL`, `TG_API_HASH`, `TG_API_ID`, `TG_SESSION_B64` |
| `refresh-topups.yml` | hourly 6-23 UTC :30 (+manual) | `FABRIC_CLIENT_SECRET` |
| `refresh-yesterday-payouts.yml` | hourly 6-23 UTC :00 (+manual) | `FABRIC_CLIENT_SECRET` |
| `retry-trustpilot.yml` | manual only | `SCRAPERAPI_KEY` |
| `test-merge-safety.yml` | push to main / pull request | — |

_28 workflows total. Generated by `scripts/generate_canonical_tables.py` — don't hand-edit between the AUTO markers._
...
**Mechanical inventory (auto-generated drift sentry).** Below is regenerated by `scripts/generate_canonical_tables.py` directly from `handleWall()` in `worker/workspace-worker.js`. If it diverges from the manual table above, fix the manual table.

<!-- AUTO:wall-endpoints BEGIN -->

| Path | Method | Notes |
|---|---|---|
| `/api/wall/whoami` | GET or POST | Returns `{ ok, email, name }` for the signed-in viewer. |
| `/api/wall/post` | POST | Body handled by `wallPost()` in `worker/workspace-worker.js`. |
| `/api/wall/comment` | POST | Body handled by `wallComment()` in `worker/workspace-worker.js`. |
| `/api/wall/react` | POST | Body handled by `wallReact()` in `worker/workspace-worker.js`. |
| `/api/wall/mark-seen` | POST | Body handled by `wallMarkSeen()` in `worker/workspace-worker.js`. |
| `/api/wall/seen-event` | POST | Body handled by `wallSeenEvent()` in `worker/workspace-worker.js`. |
| `/api/wall/upload-media` | POST | Body handled by `wallUploadMedia()` in `worker/workspace-worker.js`. |
| `/api/wall/gif-search` | POST | Body handled by `wallGifSearch()` in `worker/workspace-worker.js`. |
| `/api/wall/delete` | POST | Body handled by `wallDelete()` in `worker/workspace-worker.js`. |
| `/api/wall/edit` | POST | Body handled by `wallEdit()` in `worker/workspace-worker.js`. |
| `/api/wall/poll-vote` | POST | Body handled by `wallPollVote()` in `worker/workspace-worker.js`. |
| `/api/wall/link-preview` | POST | Body handled by `wallLinkPreview()` in `worker/workspace-worker.js`. |

_12 endpoints. Generated by `scripts/generate_canonical_tables.py` from `handleWall()` in `worker/workspace-worker.js`._

<!-- AUTO:wall-endpoints END -->
...
**Mechanical inventory (auto-generated drift sentry).** Regenerated by `scripts/generate_canonical_tables.py` from `handleHolidays()` in `worker/workspace-worker.js`.

<!-- AUTO:holidays-endpoints BEGIN -->

| Path | Method | Notes |
|---|---|---|
| `/api/holidays/whoami` | GET or POST | Returns `{ ok, email, name }` for the signed-in viewer. |
| `/api/holidays/set` | POST | Body handled by `holidaysSet()` in `worker/workspace-worker.js`. |
| `/api/holidays/seen-event` | POST | Body handled by `holidaysSeenEvent()` in `worker/workspace-worker.js`. |

_3 endpoints. Generated by `scripts/generate_canonical_tables.py` from `handleHolidays()` in `worker/workspace-worker.js`._

<!-- AUTO:holidays-endpoints END -->
...
_Generated from `pending.yaml`. Don't hand-edit this block — change the YAML and run `python3 scripts/render_pending.py`. Full detail at `/pending.html` on the site._

| ID | Title | Status | Priority | Blocker / detail |
|---|---|---|---|---|
| `holidays-v2-decision` | Holidays v2 prototype — swap, fold, or scrap | In progress | P2 | `holidays-v2.html` (year-at-a-glance lattice, GitHub-graph style) shipped overnight 2026-05-15 alongside `holidays-v2-research.md`. v1 was edited further on 2026-05-16. Decision pending: swap v2 in as the default, fold its ideas back into v1, or scrap. Until decided, v2 stays parked + unlinked from the nav. |
| `profile-google-account-lifecycle` | Profile page — Google-account lifecycle UI | In progress | P2 | After retiring the Directory inline-expand form on 2026-05-17 (`95304a3`), `profile.js` covers Role / Phone / Address / Line manager / Access level / Auth0 ID / Google accounts list / Payroll editors. Display name, Aliases, Start date, Notes, Line manager (editor), Access level (editor), Suspend toggle and Delete Person were wired the same day. **Still missing** (Google-account lifecycle UI): per-account alias management (Add alias / Remove alias / Alias → group), Convert primary account to group, Promote alt to primary (rename). Worker actions exist (`user-alias-add`, `user-alias-remove`, `alias-to-group`, `convert-to-group`, `rename-user`) — just need wiring on the profile page. |
| `auth0-chip-secrets` | Auth0 → Directory chip — awaiting Management API secrets | Shipped — awaits config | P2 | Scanner (`scan_auth0.py`) + workflow (`refresh-auth0.yml`) + chip rendering shipped 2026-05-17 (commit `014e171`). One-time setup required to activate:   1. Create Management API M2M app at https://manage.auth0.com/#/apis      (Auth0 Management API → Machine to Machine Applications → name      "TogetherBook directory reader", scope `read:users` only)   2. Paste `AUTH0_CLIENT_ID` + `AUTH0_CLIENT_SECRET` into GitHub repo secrets   3. `gh workflow run refresh-auth0.yml --ref main` Until done, every Directory row renders without the AUTH0 chip and the header banner omits the active-Auth0 count. |
| `delete-then-delayed-conversion` | Delete + 21-day delayed group conversion | Pending | P2 | Workflow the user wants: delete a primary Google account, then convert the freed email into a forwarding group 21 days later (matches Google's own grace window on the renamed primary). Needs a new pending-operations record + a scheduled run that fires the existing `convert-to-group` action against the target email + member list when the timer expires. `pending-transfers.json` + `scripts/process_pending_transfers.py` are the prior art to extend. |
| `per-api-stats` | Per-API response time + call count on home page | Blocked | P2 | Kamran Kamaei — Azure Reader grant |
| `rgroup-second-tenant` | rgroup.co.uk Workspace as second Directory tenant | Blocked | P2 | Ben Gardner — DWD setup confirm |
| `live-code-line-stats` | Live code-line stats refresh | In progress | P3 | Manual snapshot in place on `stats.html`. Either deploy `azure-function-stats/` (blocked on Azure admin) or build a `refresh-code-stats.yml` workflow following the SPEC §15 pattern using the existing DevOps PAT. |
| `brand-monitoring-stack` | Brand monitoring stack — Telegram / Discord / HIBP / Lookalike | Shipped — awaits config | P3 | Scanners + workflows + watchlists exist. Each collector's workflow is dormant until its secrets land:   Telegram: `TG_API_ID`, `TG_API_HASH`, `TG_SESSION_B64`   Discord:  `DISCORD_TOKEN`   HIBP:     `HIBP_API_KEY`   Lookalike: no secret — gated on watchlist having domains Brand activity surfaces alongside Brandwatch mentions when active. |
| `uk-bank-holidays-after-2027` | UK Bank Holidays after 2027 | Pending | P3 | `UK_BANK_HOLIDAYS` Set in `holidays.html` is hard-coded for fiscal year 2026-04 → 2027-03. When the fiscal year rolls over, append next year's nine BHs (Good Friday, Easter Monday, Early May, Spring, Summer, Christmas Day, Boxing Day, New Year's Day, Good Friday Yr+1). |
| `humand-integration` | Humand integration | Blocked | P3 | Humand support — Public API enablement + key |
| `cpf-rate-ambiguity` | Type 1 (CPF) rate=0.10/0.12 ambiguity | Blocked | P3 | User clarification |
| `null-sr1-decision` | Null-SR1 buy-vs-wait economic decision | Blocked | P3 | Upstream broker — price of null-SR1 lead |
| `reddit-oauth` | Reddit OAuth | Parked | P3 | Code wired, abandoned. Reddit dev registration is impossible via Google sign-in. ScraperAPI fallback is in use. |
| `sanitisation-pass` | Public-mirror sanitisation pass | Parked | P3 | Internal hostnames + Slack channels + payment partners + LenderId 6 + employee email pattern are still live on the page. Cloudflare Access reduces urgency but the `github.io` mirror still exposes them. User has explicitly accepted this exposure; only flag if scope changes. |
| `mirror-url-exposure` | Public mirror URL exposure | Parked | P3 | Anyone with the `richmondbot2000-prog.github.io/togetherbook/` URL reads the site without login. User has consciously accepted this. Plug = make repo private (GitHub Pro $4/mo). Re-evaluate only if scope changes. |
| `three-engineering-specs` | Three engineering specs (in wiki) — drafts only | Parked | P3 | `SPEC_AppInsights_CustomDimensions`, `SPEC_CentralStats_APIs`, `SPEC_QueueTelemetry_Tracing` in `~/Desktop/wiki/Overview/`. Need a developer + reviewer to build. CentralStats/APIs depends on customDimensions shipping first. |
| `six-audit-followups` | Six audit follow-ups from 2026-05-16 reporting audit | Parked | P3 | Disclosed on the affected pages. Listed in SPEC §16.5 "Flagged for follow-up". Each needs DB-level verification before scanner rewrites:   - broker/pipeline `paid_out` via `ApplicationStatusTypeId=5` instead     of `LeadOutcomes.LeadOutcomeTypeID=8`   - payouts UTC-vs-Eastern drift   - Brandwatch `archived_reviews` share artefact   - pipeline-samples identity-link by `(name, DOB)` only   - brokers `apply1` from Tasks-join instead of `LeadOutcomes` |
